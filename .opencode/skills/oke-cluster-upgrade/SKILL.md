...
3. **Decision Point - Version Selection**:
   ...
   - **IMPORTANT**: Present the upgrade plan to the user and get confirmation before proceeding

### Phase 2: Control-Plane Upgrade

4. **Dry-run the control-plane upgrade** (recommended):
   ```
...
5. **Execute control-plane upgrade**:
   ```
   ...
   - **Note**: Control-plane upgrade typically takes 10-20 minutes
...
7. **List node pools** to get current state:
   ```
...
8. **For each node pool requiring upgrade**:
   ...
   b. **Execute node pool configuration upgrade**:
   ```
...
9. **For each upgraded node pool, cycle the workers**:
